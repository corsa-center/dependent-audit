    ...
    def discover_dependents(self, curr_id, curr_name, curr_sha, log):
        ...
        if not self.args.no_defaults:
            ...
            if tree:
                tokens = [
                    ...
                    if curr_name.lower() in e["name"].lower()
                ]
            if not tokens:
                tokens = [(f"{curr_name}.h", False), (f"{curr_name}", True)]
        ...
        if tokens:
            for name, is_dir in tokens:
                safe = name.replace(".", "\\.")
                regex_parts.append(
                    f"include\\s*[<\\x22]{safe}/.*[>\\x22]"
                    if is_dir
                    else f"include\\s*[<\\x22]{safe}[>\\x22]"
                )
            regex_parts.append(
                f"pragma\\s+comment\\s*\\(\\s*lib\\s*,\\s*\\x22.*{curr_name}.*\\x22\\s*\\)"
            )

        if not self.args.no_defaults:
            safe_name = curr_name.replace(".", "\\.")
            ci_name = "".join(
                [f"[{c.lower()}{c.upper()}]" if c.isalpha() else c for c in safe_name]
            )
        ...
        if self.args.custom_string:
            regex_parts.append(self.args.custom_string)
        if not regex_parts:
        ...
        while page < 30:
            page += 1
            exclude_clause = (
                f" -repo:^({'|'.join([re.escape(n) for n in consumers.keys()])})$"
                if consumers
                else ""
            )
            ...
            data = self._graphql_query(
                q_search, {"query": full_query}, log, timeout=150
            )
            if not data:
                break
            ...
            log.debug(
                "Sourcegraph pagination complete",
                extra={"page": page, "new_found": new_found, "limit_hit": limit_hit},
            )
            if not limit_hit or (limit_hit and new_found == 0):
                break
...
class AuditOrchestrator:
    ...
    def run(self):
        ...
        with open(self.args.out, "w") as f:
            json.dump(
                {
                    "meta": {"root": self.args.repo, "ecosystem": self.args.ecosystem},
